...
#else
	adc_oneshot_unit_handle_t h_adc1;
    ESP_ERROR_CHECK(adc_oneshot_new_unit(&(adc_oneshot_unit_init_cfg_t) { ADC_UNIT_1 }, &h_adc1));
    adc_oneshot_chan_cfg_t config = { ADC_ATTEN_DB_0, ADC_BITWIDTH_12 };
    ESP_ERROR_CHECK(adc_oneshot_config_channel(h_adc1, ADC_CHANNEL_0, &config));
    ESP_ERROR_CHECK(adc_oneshot_config_channel(h_adc1, ADC_CHANNEL_3, &config));
#endif
	adc_ll_amp_disable();
